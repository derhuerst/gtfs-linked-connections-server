	...
	api.get(
		...
		(req, res, next) => {
			...
			if ('lc:departureTime' in req.query) {
				if (Number.isNaN(Date.parse(req.query['lc:departureTime']))) {
					return next(err(400, 'invalid departureTime parameter'))
				}
				query['lc:departureTime'] = req.query['lc:departureTime']
			}
			if ('lc:arrivalTime' in req.query) {
				if (Number.isNaN(Date.parse(req.query['lc:arrivalTime']))) {
					return next(err(400, 'invalid arrivalTime parameter'))
				}
